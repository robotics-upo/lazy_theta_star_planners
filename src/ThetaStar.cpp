					...
					float step_, float h_inflation_,  float goal_weight_, ros::NodeHandle *n)
{
	...
	Lx = ws_x_max_inflated - ws_x_min_inflated + 1;//108
    Ly = ws_y_max_inflated - ws_y_min_inflated + 1; // 156 - (-1) +1  = 158
    Lx_inv = 1.0/Lx;
    Ly_inv = 1.0/Ly;
...
void ThetaStar::setPositionYawAndTime(TrajectoryPoint &trajectory_point, Vector3 position, double _yaw, double T)
{
  tfScalar roll = 0.0;
  tfScalar pitch = 0.0;
  tfScalar yaw = _yaw;
